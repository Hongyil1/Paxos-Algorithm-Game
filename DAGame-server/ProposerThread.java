	...
	public void GetValue(int result) //Get final result from server.Learner
	{
		...
		try {
			this.outputstream.writeUTF(String.valueOf(this.Result));
			 System.out.println("Learner Sends result "+this.Result +" back to client. \n");
	...
	public void run() {
		...
		while(true) {
			...
			try {
				...
				if(line!=null) {
					System.out.println("Client input value: "+line);
					...
					System.out.println("Proposer: I'm Waitting for client.client's new Income...");
					}
				}
		...
		while(true)
			{
				...
				System.out.println("Proposer: Start new propose with N =  "+this.NumbertoPropos);
				System.out.println("Proposer: Current accepted N by Acceptor:"+this.a.CurrentAcceptedNumber);//The current accepted N value
				...
				{
					System.out.println("Proposer: I'm accepted, N = "+this.NumbertoPropos);
				try {
					Thread.sleep(Time2);
				...
				{
					System.out.println("Proposer: my Value is denied because N is too small, change N to "+this.a.CurrentAcceptedNumber);
					countD++;
					this.NumbertoPropos=this.a.CurrentAcceptedNumber;//Update N value to the current accepted N value and try again
					System.out.println("Proposer: my Value is not accepted.");
					try {
						Thread.sleep(Time3);
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
				...
				{
					System.out.println("Proposer: my N is denied because N is too small, change N to "+this.a.CurrentAcceptedNumber);
					countD++;
					this.NumbertoPropos=this.a.CurrentAcceptedNumber;//Update N value to the current accepted N value and try again
					try {
						Thread.sleep(Time3);
